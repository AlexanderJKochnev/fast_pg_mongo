...
async def test_search_codes(async_client):
    """Тест поиска Codes"""
    # Сначала создаем записи для поиска
    test_codes = [{"code": "apple_search", "url": "http://example.com/apple", "status": "pending"},
                  {"code": "banana_search", "url": "http://example.com/banana", "status": "pending"},
                  {"code": "cherry_search", "url": "http://example.com/cherry", "status": "pending"}, ]

    for code_data in test_codes:
        await async_client.post("/codes", json=code_data)

    # Ищем по полю code
    response = await async_client.get("/codes/search?query=apple&field=code&page=1&page_size=10")
    assert response.status_code == 200, response.text
    result = response.json()
    assert "items" in result
    assert "total" in result
    # Должен найти хотя бы одну запись с "apple" в code
    assert result["total"] >= 1

    # Проверяем что найденная запись содержит "apple"
    if result["items"]:
        assert "apple" in result["items"][0]["code"].lower()
